...
const Sidebar: React.FC<SideComponents> = ({ isCollapsed, onToggle }) => {
    ...
    return (
        <div className={`bg-slate-800 text-white h-screen transition-all duration-300 ease-in-out ${isCollapsed ? 'w-16' : 'w-64'
            ...
            <nav className="mt-6 space-y-3">
                {menuItems.map((item, index) => (
                    <Link
                        key={index}
                        to={item.href}
                        className={`flex items-center px-4 py-3 transition-colors group relative ${location.pathname.startsWith(item.href)
                            ? 'bg-gradient-to-r from-blue-600 to-purple-600 border-r-4 border-purple-300'
